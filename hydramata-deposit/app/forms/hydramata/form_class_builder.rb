    ...
    def build(name, config)
      form_class = Class.new do
        ...
        module_exec(self) do |module_context|
          def self.inspect
            "Hydramata::FormClassBuilder(#{work_type},#{finalizer_config})"
          end
          def self.to_s
            "Hydramata::FormClassBuilder(#{work_type},#{finalizer_config})"
          end
        ...
        def fieldsets
          @fieldsets ||= self.class.fieldsets.map {|name, attribute_names| ::Hydramata::FormFieldset.new(self, name, attribute_names)}
        end
